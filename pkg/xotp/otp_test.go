...
func TestConfig_Next(t *testing.T) {
    ...
    cfg, err := Config{}.WithKey(testKey)
    if err != nil {
...
func TestFormatAlphabet(t *testing.T) {
    ...
    t.Run("Decimal", func(t *testing.T) {
        cfg := Config{Key: "whatever", Digits: 7}
        withDefault := cfg.HOTP(12345)
        cfg.Format = FormatAlphabet(decimal)
...
func TestRFC6238Vectors(t *testing.T) {
    for _, tc := range testVectors {
        t.Run(fmt.Sprintf("%s-%d-%s", tc.alg, tc.seconds, tc.want), func(t *testing.T) {
            ...
            cfg := Config{Key: h.key, Hash: h.cons, TimeStep: fixedTime(step), Digits: len(tc.want)}
...
func ExampleConfig_customFormat() {
    ...
    cfg, err := Config{
        Digits:   SteamDigits,
        Format:   FormatAlphabet(SteamAlphabet),
