...
func NewOldPerlin(alpha, beta float64, n int32, seed int64) *OldPerlin {
    return NewOldPerlinRandSource(alpha, beta, n, rand.NewSource(seed))
}

// NewOldPerlinRandSource creates new Perlin noise generator
// In what follows "alpha" is the weight when the sum is formed.
// Typically it is 2, As this approaches 1 the function is noisier.
// "beta" is the harmonic scaling/spacing, typically 2, n is the
// number of iterations and source is source of pseudo-random int64 values
func NewOldPerlinRandSource(alpha, beta float64, n int32, source rand.Source) *OldPerlin {
    var p OldPerlin
    var i, j int32
