...
func init() {
	subcmd.Register(&chooseSTDINCommand{})
}
...
func (cs *chooseSTDINCommand) Arguments(f *flag.FlagSet) {
	...
	f.StringVar(&cs.template, "template", "", "Template to format once a selection has been made")
}
...
given video file:
   ...
   $ find . -name '*.avi' -print | cmdbox choose-stdin -exec 'xine "{}"'

See also 'cmdbox help choose-file'.`
}
...
func (cs *chooseSTDINCommand) Execute(args []string) int {
	...
	if cs.exec != "" {
		...
		//
		run := stemplate.Expand(cs.exec, choice, "")
		...
		fmt.Println(stemplate.ExpandString(cs.template, choice))
		return 0
	}
