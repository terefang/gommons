...
func (p *passwordCommand) Arguments(f *flag.FlagSet) {
    ...
    f.BoolVar(&p.doKdf, "kdf", false, "use kdf to scramble given password")
...
func (p *passwordCommand) Execute(args []string) int {
    ...
    if p.doKdf {
        p.password = xcrypt.GeneratePasswordWithKdfSymLevel(p.password, symbolset, p.length, len(symbolset))
        fmt.Printf("%s\n", p.password)
        buf = []byte(p.password)
    }
