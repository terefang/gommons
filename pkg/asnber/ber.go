...
const (
	...
	DerInteger     BERType = AsnUniversal | 0x02
	DerInteger32   BERType = AsnUniversal | 0x02
	DerBitstring   BERType = AsnUniversal | 0x03
	...
	SnmpCounter32 BERType = AsnApplication | 0x01
	SnmpGauge     BERType = AsnApplication | 0x02
	SnmpGauge32   BERType = AsnApplication | 0x02
	SnmpTimeticks BERType = AsnApplication | 0x03
	SnmpOpaque    BERType = AsnApplication | 0x04
	SnmpCounter64 BERType = AsnApplication | 0x06

	SnmpGetRequest     BERType = 0xa0
	SnmpGetNextRequest BERType = 0xa1
	SnmpGetResponse    BERType = 0xa2
	SnmpSetRequest     BERType = 0xa3
	SnmpTrapV1         BERType = 0xa4
	SnmpGetBulkRequest BERType = 0xa5
	SnmpTrapV2         BERType = 0xa7

	SnmpNoSuchInstance BERType = 0x81
	SnmpEndOfMibView   BERType = 0x82
)
...
func DecodeSequence(toparse []byte) ([]interface{}, error) {
	...
	if sqType != DerSequence && (toparse[0]&0x20 == 0) {
	...
	for idx < len(toparse) && idx > lidx {
		...
		case SnmpGetNextRequest, SnmpGetRequest, SnmpGetResponse, SnmpTrapV2:
			pdu, err := DecodeSequence(berAll)
			if err != nil {
				return nil, err
			}
		...
		case SnmpEndOfMibView:
			result = append(result, SnmpEndOfMibView)
		default:
...
func EncodeSequence(toEncode []interface{}) ([]byte, error) {
	...
	for _, val := range toEncode[1:] {
		...
		case Counter:
			...
			// TODO encode length ?
			toEncap = append(toEncap, byte(SnmpCounter32))
			toEncap = append(toEncap, byte(len(enc)))
			for _, b := range enc {
				toEncap = append(toEncap, b)
			}
		case Gauge:
			enc := EncodeUInt(uint64(val))
			// TODO encode length ?
			toEncap = append(toEncap, byte(SnmpGauge32))
			toEncap = append(toEncap, byte(len(enc)))
			for _, b := range enc {
				toEncap = append(toEncap, b)
			}
		...
		case net.IP:
			...
			enc := []byte(valIPv4)
			toEncap = append(toEncap, byte(SnmpIpaddress))
			toEncap = append(toEncap, byte(len(enc)))
			for _, b := range enc {
				toEncap = append(toEncap, b)
			}
