...
func TestEncoding(t *testing.T) {
    ...
    for _, test := range tests {
        t.Run("String", func(t *testing.T) {
            got := test.OtpUrl.String()
            if got != test.want {
                t.Errorf("Input: %+v\nWrong encoding:\n got: %q\nwant: %q", test.OtpUrl, got, test.want)
            }
        })
        t.Run("Text", func(t *testing.T) {
            // The URL should encode to the same format as String.
            text, err := test.OtpUrl.MarshalText()
            if err != nil {
            ...
            if got, want := cmp.String(), test.OtpUrl.String(); got != want {
